...
LOCKFILE="/tmp/flokshell_sleep_monitor.lock"
if [ -e "$LOCKFILE" ]; then
	PID=$(cat "$LOCKFILE")
...
CONFIG_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/flokshell/config/system.json"
...
get_cmd() {
	...
	if [ -f "$CONFIG_FILE" ]; then
		if [ "$type" == "before" ]; then
			...
			jq -r '.idle.general.after_sleep_cmd // "flok screen on"' "$CONFIG_FILE"
		fi
	else
		if [ "$type" == "before" ]; then
