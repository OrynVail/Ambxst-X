#!/usr/bin/env bash
# Flokshell CLI - It was needed, so here it is. lol
...
QS_BIN="${FLOKSHELL_QS:-qs}"
NIXGL_BIN="${FLOKSHELL_NIXGL:-}"
...
ensure_config_files() {
	local config_dir="${XDG_CONFIG_HOME:-$HOME/.config}/flokshell/config"
	local preset_dir="${SCRIPT_DIR}/assets/presets/Flokshell Default"
...
show_help() {
	cat <<EOF
Flokshell CLI - Desktop Environment Control

Usage: flok [COMMAND]

Commands:
    (none)                            Launch Flokshell
    update                            Update Flokshell
    ...
    version, -v, --version            Show Flokshell version
    goodbye                           Uninstall Flokshell :(
    install <target>                    Install compositor config (hyprland)
    remove <target>                    Remove compositor config (hyprland)

Examples:
    flok brightness 75              Set all monitors to 75%
    flok brightness 50 HDMI-A-1     Set HDMI-A-1 to 50%
    flok brightness +10             Increase brightness by 10%
    flok brightness -5 HDMI-A-1     Decrease HDMI-A-1 brightness by 5%
    flok brightness 10 -s           Save current, then set all to 10%
    flok brightness -s HDMI-A-1     Save current brightness of HDMI-A-1
    flok brightness -r              Restore saved brightness

EOF
}

FLOKSHELL_HYPR_CONF_SOURCE="source = ~/.local/share/flokshell/hyprland.conf"
FLOKSHELL_HYPR_LUA_SOURCE='loadfile(os.getenv("HOME") .. "/.local/share/flokshell/hyprland.lua")()'
FLOKSHELL_HYPR_CONF_BLOCK=$(
	cat <<'EOF'
# Flokshell
source = ~/.local/share/flokshell/hyprland.conf

# OVERRIDES
# Down here you can write or source anything that you want to override from Flokshell's settings.
EOF
)
FLOKSHELL_HYPR_LUA_BLOCK=$(
	cat <<'EOF'
-- Flokshell
loadfile(os.getenv("HOME") .. "/.local/share/flokshell/hyprland.lua")()
...
append_flokshell_hyprland_block() {
	local conf="$1"
	local source="$2"
	local block="$3"

	if [ -f "$conf" ] && grep -qF "$source" "$conf"; then
		echo "Flokshell Hyprland block already present in $conf"
		return 0
	fi
	...
	echo "Added Flokshell Hyprland block to $conf"
}

remove_flokshell_hyprland_block() {
	local conf="$1"
	local source="$2"
	...
	awk -v source="$source" '
		function is_remove(line) {
			return line == source \
				...
				|| line == "# OVERRIDES" \
				|| line == "-- OVERRIDES" \
				|| line == "# Down here you can write or source anything that you want to override from Flokshell'\''s settings." \
				|| line == "-- Down here you can write or source anything that you want to override from Flokshell'\''s settings."
		}
		{
			lines[NR] = $0
	...
	echo "Removed Flokshell Hyprland block from $conf"
}
...
restart_flokshell() {
	...
	PID=$(find_flokshell_pid_cached)
	if [ -n "$PID" ]; then
		echo "Stopping Flokshell (PID $PID)..."
		kill "$PID"
...
case "${1:-}" in
update)
	echo "Updating Flokshell..."
	curl -fsSL https://raw.githubusercontent.com/OrynVail/flokshell/main/install.sh | sh
	restart_flokshell
	;;
refresh)
	echo "Refreshing Flokshell profile..."
	exec nix profile upgrade Flokshell --refresh --impure
	;;
run)
	CMD="${2:-}"
	PIPE="/tmp/flokshell_ipc.pipe"
	...
	PID=$(find_flokshell_pid_cached)
	if [ -z "$PID" ]; then
		echo "Error: Flokshell is not running"
		exit 1
	fi

	qs ipc --pid "$PID" call flokshell run "$CMD" 2>/dev/null || {
...
lock)
	...
	qs ipc --pid "$PID" call flokshell run lockscreen 2>/dev/null || {
...
brightness)
	...
	BRIGHTNESS_SAVE_FILE="/tmp/flokshell_brightness_saved.txt"
	...
	if [[ "$ARG2" =~ ^[0-9]+$ ]]; then
		...
		echo "Run 'flok help' for usage information"
		exit 1
	fi
	...
	;;
version | -v | --version)
	echo "Flokshell $(cat "${SCRIPT_DIR}/version")"
	;;
install)
	TARGET="${2:-}"
	if [ "$TARGET" = "hyprland" ]; then
		...
		if [ -f "$HYPR_LUA" ] || [ ! -f "$HYPR_CONF" ]; then
			append_flokshell_hyprland_block "$HYPR_LUA" "$FLOKSHELL_HYPR_LUA_SOURCE" "$FLOKSHELL_HYPR_LUA_BLOCK"
		else
			append_flokshell_hyprland_block "$HYPR_CONF" "$FLOKSHELL_HYPR_CONF_SOURCE" "$FLOKSHELL_HYPR_CONF_BLOCK"
		fi
	else
...
remove)
	...
	if [ "$TARGET" = "hyprland" ]; then
		...
		remove_flokshell_hyprland_block "$HYPR_LUA" "$FLOKSHELL_HYPR_LUA_SOURCE"
		remove_flokshell_hyprland_block "$HYPR_CONF" "$FLOKSHELL_HYPR_CONF_SOURCE"
	else
		echo "Error: Unknown target '$TARGET'. Supported: hyprland"
		exit 1
	fi
...
goodbye)
	...
	if [ -f /etc/NIXOS ]; then
		if nix profile list 2>/dev/null | grep -q "Flokshell"; then
			echo "Removing from nix profile..."
			nix profile remove Flokshell
		elif command -v flok >/dev/null 2>&1; then
			echo "Flokshell was declared in this system. Please remove it from your configuration in order to uninstall."
		else
			echo "Flokshell is not installed."
		fi
		exit 0
	fi
	...
	rm -rf "$HOME/.local/src/flokshell"
	rm -rf "$HOME/.local/share/flokshell"
	rm -rf "$HOME/.local/state/flokshell"

	if [ "$REMOVE_CONFIG" = true ]; then
		rm -rf "$HOME/.config/flokshell"
		echo "Configuration files removed."
	fi

	echo "Flokshell uninstalled. :("
	;;
help | --help | -h)
	show_help
	;;
