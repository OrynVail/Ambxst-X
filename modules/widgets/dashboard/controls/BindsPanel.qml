...
Item {
    ...
    property string currentCategory: "flokshell"
    ...
    property bool isEditingFlokshell: false
    ...
    function openEditDialog(bind, index, isFlokshell) {
        root.editingIndex = index;
        root.editingBind = bind;
        root.isEditingFlokshell = isFlokshell;

        // Initialize edit form state
        if (isFlokshell) {
            // Flokshell binds still use old format (single key)
    ...
    function saveEdit() {
        if (root.isEditingFlokshell) {
            // Save flokshell bind (still uses old format internally)
            const path = root.editingBind.path.split(".");
            // path = ["flokshell", "section"?, "bindName"]
            
            const adapter = Config.keybindsLoader.adapter;
            if (adapter && adapter.flokshell) {
                let bindObj = null;
                if (path.length === 2) {
                    // Top level: flokshell.bindName
                    bindObj = adapter.flokshell[path[1]];
                } else if (path.length === 3) {
                    // Nested: flokshell.system.bindName
                    bindObj = adapter.flokshell[path[1]][path[2]];
                }
    ...
    // Get flokshell binds as a flat list
    function getFlokshellBinds() {
        const adapter = Config.keybindsLoader.adapter;
        if (!adapter || !adapter.flokshell)
            return [];

        const binds = [];
        const flokshell = adapter.flokshell;

        // Core Flokshell binds (Launcher, Dashboard, etc.)
        ...
        if (flokshell.system) {
            ...
            for (const key of systemKeys) {
                if (flokshell.system[key]) {
                    binds.push({
                        category: "System",
                        name: key.charAt(0).toUpperCase() + key.slice(1),
                        path: "flokshell.system." + key,
                        bind: flokshell.system[key]
                    });
                }
            }
        }
    ...
    Flickable {
        ...
        ColumnLayout {
            ...
            // Flokshell binds view
            Repeater {
                id: flokshellRepeater
                model: root.currentCategory === "flokshell" ? root.getFlokshellBinds() : []
            ...
            Text {
                ...
                Layout.topMargin: 20
                visible: (root.currentCategory === "flokshell" && flokshellRepeater.count === 0) || (root.currentCategory === "custom" && customRepeater.count === 0)
                text: root.currentCategory === "flokshell" ? "No Flokshell binds configured" : "No custom binds configured"
                font.family: Config.theme.font
                font.pixelSize: Styling.fontSize(0)
            ...
            ColumnLayout {
                ...
                Item {
                    ...
                    RowLayout {
                        ...
                        StyledRect {
                            id: deleteButton
                            visible: !root.isEditingFlokshell && !root.isCreatingNew
                        ...
                        // Reset button (only for Flokshell binds)
                        StyledRect {
                            id: resetButton
                            visible: root.isEditingFlokshell
                            ...
                            MouseArea {
                                ...
                                cursorShape: Qt.PointingHandCursor
                                onClicked: {
                                    if (root.isEditingFlokshell && root.editingBind) {
                                        const path = root.editingBind.path.split(".");
                                        // path = ["flokshell", "dashboard"|"system", "bindName"]
                                        const section = path[1];
                                        const bindName = path[2];
                                        
                                        // Use the new helper in Config.qml to get the default values
                                        const defaultBind = Config.keybindsLoader.adapter.getFlokshellDefault(section, bindName);
                ...
                Item {
                    ...
                    ColumnLayout {
                        ...
                        // Bind name/info (for flokshell binds only)
                        Text {
                            visible: root.isEditingFlokshell && root.editingBind !== null
                        ...
                        ColumnLayout {
                            ...
                            RowLayout {
                                ...
                                StyledRect {
                                    id: removeKeyBtn
                                    visible: root.editKeys.length > 1 && !root.isEditingFlokshell
                        ...
                        // =====================
                        // ACTIONS SECTION (custom binds & flags for flokshell)
                        // =====================
                        ColumnLayout {
                            Layout.fillWidth: true
                            spacing: 8
                            // visible: !root.isEditingFlokshell - Removed to allow editing flags for Flokshell binds
                            ...
                            RowLayout {
                                ...
                                // Page indicator
                                Text {
                                    visible: root.editActions.length > 1 && !root.isEditingFlokshell
    ...
    component BindItem: StyledRect {
        ...
        property bool isFlokshell: true
        property bool isHovered: false
