...
# === Configuration ===
REPO_URL="https://github.com/OrynVail/flokshell.git"
INSTALL_PATH="$HOME/.local/src/flokshell"
...
install_dependencies() {
  case "$DISTRO" in
  nixos)
    local FLAKE_URI="${1:-github:OrynVail/flokshell}"
    nix profile list | grep -q "ddcutil" && nix profile remove ddcutil 2>/dev/null || true

    if nix profile list | grep -q "Flokshell"; then
      log_info "Updating Flokshell..."
      nix profile upgrade Flokshell --refresh --impure
    else
      log_info "Installing Flokshell..."
...
# === Migration ===
migrate_old_paths() {
  log_info "Checking for old Flokshell paths..."

  # Source migration (PascalCase -> lowercase)
  local OLD_SRC="$HOME/Flokshell"
  ...
  local NEW_CONFIG="$HOME/.config/flokshell"
  ...
  local NEW_SHARE="$HOME/.local/share/flokshell"
  ...
  local NEW_STATE="$HOME/.local/state/flokshell"
  ...
  local NEW_CACHE_DIR="$HOME/.cache/flokshell"
  ...
  local NEW_CACHE="$HOME/.cache/flokshell"
  if [[ -d "$NEW_SHARE" ]]; then
    mkdir -p "$NEW_CACHE"
  ...
  if [[ -f "$NEW_CONFIG/config.json" && ! -d "$NEW_CONFIG/config" ]]; then
    ...
    log_info "Flokshell now uses a multi-file configuration in $NEW_CONFIG/config/"
...
setup_repo() {
  ...
  if [[ ! -d "$INSTALL_PATH" ]]; then
    log_info "Cloning Flokshell to $INSTALL_PATH..."
    mkdir -p "$(dirname "$INSTALL_PATH")"
...
setup_launcher() {
  ...
  [[ -f "$HOME/.local/bin/flok" ]] && rm -f "$HOME/.local/bin/flok"

  sudo mkdir -p "$BIN_DIR"
  local LAUNCHER="$BIN_DIR/flok"
...
[[ "$DISTRO" != "nixos" ]] && echo -e "Run ${GREEN}flokshell${NC} to start."
